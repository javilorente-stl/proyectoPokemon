...
public class MovimientoCrud {
	...
	public static Movimiento obtenerMovimientoInicial(Connection con, Tipo tipoPrincipal) throws SQLException {
		...
		String sql = "SELECT * FROM MOVIMIENTO " + "WHERE TIPO = ? AND POTENCIA > 0 " + "ORDER BY POTENCIA ASC "
				+ "LIMIT 1";
		...
		try (PreparedStatement ps = con.prepareStatement(sql)) {
			...
			System.err.println(
					"Error al obtener el movimiento inicial para tipo " + tipoPrincipal + ": " + e.getMessage());
			throw e;
		}
	...
	public static void gestionarAprendizajeUI(Connection con, Pokemon pk) throws SQLException {
		...
		if (pk.getMovimientos().size() < 4) {
			...
			Movimiento aOlvidar = (Movimiento) JOptionPane.showInputDialog(null,
					"Selecciona el movimiento que quieres eliminar para siempre:", "Sustituir Movimiento",
					JOptionPane.WARNING_MESSAGE, null, pk.getMovimientos().toArray(), pk.getMovimientos().get(0));
