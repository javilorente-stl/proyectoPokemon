...
public class CajaController {
    ...
    public void recibirDatos(Entrenador ent) throws SQLException {
        ...
        try (Connection conexion = conBD.getConnection()) {
            if (conexion != null) {
                // 1. Mantenemos la lógica de cargar el equipo actual
                ...
                // 2. NUEVO: Cargar todos los Pokémon (Caja + Equipo)
                // Necesitarás un método en CRUD que devuelva una lista completa
                LinkedList<Pokemon> pokemonEnCaja = e.getEquipo2();
                
                // 3. Dibujamos el Grid con todos los Pokémon
                rellenarCaja(pokemonEnCaja);
            }
        }
        
        // 4. Mostramos por defecto el primer Pokémon del equipo en el panel de stats
    ...
    private void gestionarMoverAEquipo(Pokemon pOrigen) {
        // 1. Verificar si hay hueco en el equipo (menos de 6 Pokémon)
        if (e.getEquipo1().size() < 6) {
            // ESCENARIO A: Movimiento directo (el equipo tiene espacio)
            moverDirectoAEquipo(pOrigen);
        } else {
            // ESCENARIO B: El equipo está lleno, abrimos el selector de intercambio
    ...
    private void abrirSelectorIntercambio(Pokemon pOrigen) {
        try {
            ...
            // ACTIVAMOS el método que creamos antes en el EquipoController
            // Esto le pasa el Pokémon de la caja y activa el modo de cierre automático
