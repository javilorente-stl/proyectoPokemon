...
public class Combate {
    ...
    public void aplicarEfectosFinales(Pokemon p) {
        if (p.getVitalidad() <= 0 || p.getEstado() == Estado.VIVO) return;
        ...
        if (estado == Estado.ATRAPADO || estado == Estado.CONFUSO || estado == Estado.DRENADORAS) {
            ...
            if (esJugador) this.turnosEstadoJugador = turnosRestantes;
            else this.turnosEstadoRival = turnosRestantes;
        ...
        if (danioResidual <= 0 && estado != Estado.VIVO) {
            ...
            if(estado == Estado.QUEMADO || estado == Estado.ENVENENADO || 
               estado == Estado.MALDITO || estado == Estado.DRENADORAS) {
                danioResidual = 1;
            }
        }
    ...
    private void mejorasAtaque(Movimiento mov, Pokemon atacante, Pokemon defensor) {
        ...
        for (int i = 1; i <= repeticiones; i++) {
            ...
            if ("Fijo".equalsIgnoreCase(mejora)) {
                ...
                if (multCritico > 1.0) registrarSuceso("¡Un golpe crítico!");
            ...
            if (defensor.getVitalidad() <= 0) break;
        }
    ...
    private void aplicarEstadoAtaque(Movimiento mov, Pokemon defensor) {
        ...
        if (rand.nextDouble() < 0.15) {
            ...
            // Si el movimiento no tiene estado asociado o el rival ya está debilitado, salimos
            ...
            if (estadoAAplicar == Estado.ATRAPADO) {
                ...
                if (defensor == this.pJugador) this.turnosEstadoJugador = duracion;
                else this.turnosEstadoRival = duracion;
            ...
            //  Lógica para el resto de estados persistentes
                ...
                if (defensor.getEstado() == Estado.VIVO) {
                    ...
                    registrarSuceso(defensor.getNombre() + " ahora está " + estadoAAplicar.toString().toLowerCase() + ".");
                } else {
            ...
            // El 85% de las veces el estado no se aplica, pero el log no necesita registrar el fallo
    ...
    private void aplicarEstadoClase2(Movimiento mov, Pokemon defensor) {
        ...
        if (duracion > 0) {
            ...
            defensor.setEstado(estadoAAplicar);
            registrarSuceso(defensor.getNombre() + " sufre " + estadoAAplicar.name().toLowerCase() + " por " + duracion + " turnos.");
        } 
    ...
    public Pokemon getGanador() {
        if (pJugador.getVitalidad() > 0 && pRival.getVitalidad() <= 0) return pJugador;
        if (pRival.getVitalidad() > 0 && pJugador.getVitalidad() <= 0) return pRival;
